...
def load_matches_data(league_folder, season):
    ...
    folder_path = os.path.join("/workspaces/streamlit-example", "Data", f"{league_folder}", f"{season}", "match-data")
...
def load_events_df(matches_data):
    ...
    directory = os.path.join("/workspaces/streamlit-example", f'Data/{comp}/{season[5:]}/raw-season-data/')
    pattern = f'{comp}-*.csv'
...
def process_and_export_match_data(events_df, matches_data, season, comp):
    ...
    for matchId in matchIds:
        ...
        team_df.to_csv(f'/workspaces/streamlit-example/Data/{comp}/{season}/raw-season-data/{home}-{away}-{date}-passes-carries.csv')
        dfs.append(df)
...
def load_individual_match_team_dfs(comp, season, pass_filter='passes-carries'):
    ...
    path_to_folder = os.path.join("/workspaces/streamlit-example", f'Data/{comp}/{season[5:]}/raw-season-data/')
