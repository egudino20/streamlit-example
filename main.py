...
def createEventsDF(data):
    ...
    for i in events_df.loc[events_df.isShot==True].index:
        for j in events_df.loc[events_df.isShot==True].qualifiers.loc[i]:
            if j['type'] == 'FromCorner' or j['type'] == 'SetPiece' or j['type'] == 'DirectFreekick' or j['type'] == 'penalty':
...
def insert_ball_carries(events_df, min_carry_length=3, max_carry_length=60, min_carry_duration=1, max_carry_duration=10):
    ...
    events_df['second_frac'] = events_df['second'] / 60
    events_df['cumulative_mins'] = events_df['minute'] + events_df['second_frac']
    ...
    for match_id in events_df['matchId'].unique():
        ...
        for idx, match_event in match_events.iterrows():
            ...
            if idx < len(match_events) - 1:
                ...
                valid_carry = same_team & same_period & not_ball_touch & far_enough & not_too_far & min_time & same_phase
    ...
    #return match_events_and_carries.type.unique()
    return events_out
